...
bool rysev_m_gypercube::GyperCube::validation() {
  internal_order_test();
  if (world.size() < 2 || taskData->inputs.empty()) return false;
  return true;
}
...
bool rysev_m_gypercube::GyperCube::run() {
  ...
  for (int i = 0; i < std::log2(world.size()); i++){	  
	...
	if (neighbr < world.size()) {
	  ...
	  if (taskData->outputs_count[0] >= to_recv.size()) { std::copy(to_recv.begin(), to_recv.begin() + to_recv.size(), taskData->outputs[0]); }
      else { return false; }
	}
	world.barrier();
  }
